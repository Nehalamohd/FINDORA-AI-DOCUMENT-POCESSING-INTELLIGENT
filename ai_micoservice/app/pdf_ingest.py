"""
Utility module for ingesting PDF documents, including text extraction,
vision-based fallback for scanned documents, and chunking for RAG.
"""
...
def extract_text_by_page(pdf_path: str):
    """
    Extracts text from a PDF file page by page using PyPDF2.
...
def chunk_text(text, size=500):
    """
    Splits long text into smaller chunks for LLM processing.
    
    Args:
        text (str): The input text to be chunked.
        size (int, optional): The maximum number of words per chunk. Defaults to 500.
...
def process_pdf_page(document_id: int, page_no: int, file_path: str):
    ...
    try:
        ...
        try:
            ...
            logger.warning(f"Direct text extraction failed for page {page_no} of {file_path}: {str(e)}")
            extracted_text = ""

        # 2. Fallback to Vision if text is too sparse
        if len(extracted_text) < 50:
            ...
            try:
                ...
                logger.error(f"Vision fallback failed for page {page_no} of {file_path}: {str(e)}")

        if not extracted_text:
            logger.warning(f"No text extracted/found for page {page_no} of doc {document_id}")
            return None
        ...
        try:
            ...
            db.refresh(page)
            page_id = page.id
            logger.debug(f"Saved page {page_no} for document {document_id}")
            ...
            logger.error(f"Database error saving page {page_no} data for doc {document_id}: {str(db_e)}")
            db.rollback()
            return None
    except Exception as general_e:
        logger.error(f"System failure processing page {page_no} for doc {document_id}: {str(general_e)}")
        return None
    finally:
        db.close()
...
def ingest_pdf(file_path: str, filename: str, task_id: str = None, flow_id: str = None, username: str = None):
    """
    Orchestrates the full ingestion process for a PDF file.
    
    Args:
        file_path (str): Path to the PDF file.
        filename (str): Original name of the file.
        task_id (str, optional): Celery task ID for status tracking.
        flow_id (str, optional): UUID of the flow this document belongs to.
        username (str, optional): Username of the owner.
        
    Returns:
        int: The database ID of the created Document.
        
    Raises:
        Exception: If document registration or processing fails.
    """
    db = SessionLocal()
    try:
        doc = Document(
